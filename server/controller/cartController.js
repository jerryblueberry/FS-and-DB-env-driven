...
const cartDecreament   = asyncHandler(async(req,res) => {
  ...
  try {
    ...
    if(index !== -1){
      const existingProductIndex = carts[index].products.findIndex((item) => item.product.toString() === product.toString());

      if(existingProductIndex !== -1){
        if(carts[index].products[existingProductIndex].quantity <=1){
          return res.json({message:"Quantity cannot be less than 1 Instead try Removing it!"})
        }
        carts[index].products[existingProductIndex].quantity -=1;

        carts[index].total_price  = carts[index].products.reduce((total,product) => total+product.price * product.quantity,0);
...
const removeProduct = asyncHandler(async(req,res) => {
  ...
  try {
    ...
    if(index !== -1){
      const productIndex = carts[index].products.findIndex((item) => item.product.toString() === product.toString());
